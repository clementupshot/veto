...
bouncer integrates via PATH shims rather than a custom hook script.

## Setup

```sh
bouncer install-codex                # install-shims + codex-config scan
export PATH=$HOME/.local/bin:$PATH   # in front of the real npm/pip/... dirs
```

`bouncer install-codex` is `install-shims` plus an inspection of
...
Layer 3 (`bouncer install-preload`) and Layer 4
(`bouncer install-wrappers`). Layer 3 hooks libc's execve in any
...
files that aren't already bouncer-managed symlinks unless `--force` is
passed.
